"""Collect and filter UK Tech (non-FTSE) organization repositories by recent activity with health metrics."""
...
class UKTechActiveReposCollector:
    """Collect active repositories from UK Tech (non-FTSE) organizations with health metrics."""
    ...
    def __init__(self, days_threshold: int = 30):
        ...
        self.input_file = Path(__file__).parent.parent / 'data' / 'orgs' / 'index' / 'pull' / 'UK-tech-nonFTSE-claude.csv'
        self.output_file = Path(__file__).parent.parent / 'data' / 'repos' / 'batch_Aug28' / 'uktech_active_repos_sorted_by_health.csv'
        self.results = []
    ...
    def get_org_repositories_filtered(self, org_name: str, company_name: str) -> List[Dict]:
        """Get repositories for an organization filtered by recent activity with health metrics."""
        logger.info(f"Fetching repositories for {company_name} ({org_name})...")
        ...
        try:
            ...
            logger.info(f"Found {active_count} active repositories (last {self.days_threshold} days) for {company_name}")
    ...
    def collect_all_organizations(self):
        """Process all organizations from the UK Tech CSV."""
        ...
        for i, org_info in enumerate(orgs_to_process, 1):
            ...
            repos = self.get_org_repositories_filtered(org_info['org_name'], org_info['company'])
    ...
    def print_summary(self):
        ...
        print("\n" + "="*60)
        print("UK TECH (NON-FTSE) ACTIVE REPOSITORIES SUMMARY")
        print("="*60)
    ...
    def run(self):
        ...
        logger.info(f"Starting UK Tech (non-FTSE) active repositories collection with health metrics")
...
def main():
    ...
    collector = UKTechActiveReposCollector(days_threshold=30)
    collector.run()
